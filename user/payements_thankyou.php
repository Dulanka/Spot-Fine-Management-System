...
if (isset($_SESSION['license_id']) && isset($_SESSION['driver_email']) && isset($_SESSION['driver_name']) && isset($_SESSION['home_address'])) {
        ...
        // Make sure 'ref_id' is set in the POST data
      //  if (isset($_POST['ref_no'])) {
            
            $desiredRefId = $_POST['ref_no'];
            include "../connection.php";	
            $sql=mysqli_query($conn,"UPDATE issued_fines SET status = 'paid' WHERE ref_no ='$desiredRefId'");
          /*  if ($res=mysqli_fetch_assoc($sql)) {
                $updatePass = mysqli_query($conn, $sql) or die("Query Failed");
  */
          ?>
