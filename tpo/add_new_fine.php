...
if (isset($_POST['search']))
{
	...
	if(empty($dlno)){
        ...
        $sql = mysqli_query($conn, "SELECT * FROM driver WHERE license_id = '$dlno' AND points > 0");
		...
		{
            header("Location: add_new_fine.php?error= Invalid License ID! OR Suspended License");
			exit();
		} 
    }	
}
...
//---------------------------------------issue fine code goes here ---------------------------------------------------------------
	...
	if (empty($license)) {
    ...
    else
     {
		...
		$sql3 = "UPDATE driver SET points = points - 1 WHERE license_id = '$license'";
